...
class Zend_Service_Rackspace_Servers extends Zend_Service_Rackspace_Abstract
{
    ...
    {
        if (empty($id)) {
            throw new InvalidArgumentException(self::ERROR_PARAM_NO_SERVERID);
        }
    ...
    public function createServer(array $data, $metadata = array(), $files = array())
    {
        if (empty($data) || !is_array($data) || !is_array($metadata) || !is_array($files)) {
            throw new InvalidArgumentException(self::ERROR_PARAM_NO_ARRAY);
        }
        if (!isset($data['name'])) {
            throw new InvalidArgumentException(self::ERROR_PARAM_NO_NAME);
        }
        if (!isset($data['flavorId'])) {
            throw new InvalidArgumentException(self::ERROR_PARAM_NO_FLAVORID);
        }
        if (!isset($data['imageId'])) {
            throw new InvalidArgumentException(self::ERROR_PARAM_NO_IMAGEID);
        }
        if (count($files) > self::LIMIT_NUM_FILE) {
            throw new InvalidArgumentException('You can attach ' . self::LIMIT_NUM_FILE . ' files maximum');
        }
        if (!empty($metadata)) {
    ...
    {
        if (empty($id)) {
            throw new InvalidArgumentException('You must specify the ID of the server');
        }
        if (empty($name) && empty($password)) {
             throw new InvalidArgumentException("You must specify the new name or password of server");
        }
        $data = array();
        if (!empty($name)) {
            $data['name'] = $name;
    ...
    public function changeServerName($id, $name)
    {
        ...
        if (empty($name)) {
             throw new InvalidArgumentException("You must specify the new name of the server");
        }
    ...
    public function changeServerPassword($id,$password)
    {
        ...
        if (empty($password)) {
             throw newInvalidArgumentException("You must specify the new password of the server");
        }
    ...
    public function shareIpAddress($id, $ip, $groupId, $configure = true)
    {
        ...
        if (empty($ip)) {
            throw new InvalidArgumentException('You didn\'t specified the IP address to share');
        }
        $validator = new Zend_Validate_Ip();
        if (!$validator->isValid($ip)) {
            throw new InvalidArgumentException("The parameter $ip specified is not a valid IP address");
        }
        if (empty($groupId)) {
            throw new InvalidArgumentException('You didn\'t specified the group id to use');
        }
        $data= array (
    ...
    public function rebuildServer($id, $imageId)
    {
        ...
        if (empty($imageId)) {
            throw new InvalidArgumentException('You didn\'t specified the new imageId of the server');
        }
        $data= array (
            'rebuild' => array (
    ...
    public function resizeServer($id, $flavorId)
    {
        ...
        if (empty($flavorId)) {
            throw new InvalidArgumentException('You didn\'t specified the new flavorId of the server');
        }
        $data= array (
            'resize' => array (
    ...
    {
        if (empty($id)) {
            throw new InvalidArgumentException(self::ERROR_PARAM_NO_ID);
        }
    ...
    public function getBackupSchedule($id)
    {
        ...
        $result = $this->httpCall($this->getManagementUrl() . '/servers/' . rawurlencode($id) . '/backup_schedule', 'GET');
        $status = $result->getStatus();
        switch ($status) {
            case '200' :
            case '203' : // break intentionally omitted
    ...
    public function changeBackupSchedule($id, $weekly, $daily)
    {
        ...
        if (empty($weekly)) {
            throw new InvalidArgumentException(self::ERROR_PARAM_NO_WEEKLY);
        }
        if (empty($daily)) {
            throw new InvalidArgumentException(self::ERROR_PARAM_NO_DAILY);
        }
        $data = array (

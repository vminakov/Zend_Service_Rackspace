...
class Zend_Service_Rackspace_Abstract
{
	 ...
	 * @param Zend_Http_Client|null 
     * @throws Zend_Service_Rackspace_Exception
     */
    public function __construct($user, $key, $authUrl = self::US_AUTH_URL, Zend_Http_Client $httpClient = null)
    {
        if (!isset($user)) {
        ...
        $this->setHttpClient($httpClient ? $httpClient : new Zend_Http_Client);
    }

	/**
     * Set HTTP client to use for all connections
	 * 
	 * @param Zend_Http_Client $httpClient
	 * 
     * @return Zend_Service_Rackspace_Abstract
     */
    ...
    public function getManagementUrl()
    {
        if (empty($this->managementUrl)) {
            if (!$this->authenticate()) {
                throw new Zend_Service_Rackspace_Exception('Authentication failed, you need a valid token to use the Rackspace API');
            }
        }
